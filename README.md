...
GitHub Actions 发布需要以下 Secrets，否则会出现 `Input required and not supplied: apiToken` 报错：

- `CF_PAGES_TOKEN`：Cloudflare API Token，至少需要 Pages 的发布权限
- `CF_ACCOUNT_ID`：Cloudflare 账户 ID
- `CF_PROJECT_NAME`：Cloudflare Pages 项目名称（**Pages 控制台里显示的项目名**，不是自定义域名）

可在 GitHub 仓库 **Settings → Secrets and variables → Actions** 中新增以上变量。
